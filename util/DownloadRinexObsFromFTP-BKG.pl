...
# Download OBS RINEX v303 from Bundesamt für Kartographie und Geodäsie (BKG)
# file repository.
# ---------------------------------------------------------------------------- #
...
# - $1 -> Year
# - $2 -> Day of year
# - $3 -> station (Site or Site ID)
# - $4 -> storage path

# TODO: Add consistency check for input arguments:
my ($year, $doy, $station, $storage_path) = @ARGV;
...
my $rinex_pattern = $station.'*'.$year.$doy.'*O.crx.gz';;
