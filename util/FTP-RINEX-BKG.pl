...
# CDDIS ftp path:
my $ftp_parent = qw(ftp://igs.bkg.bund.de/IGS/obs/);
...
my @stations = split(/\s+|,\s*/,  $stations_content);

# Stations names to upper case:
@stations = map(uc, @stations);
...
foreach my $station (@stations) {
  ...
  $rinex_list{$station} = (-e $rinex) ? qw(DOWNLOADED) : qw(NOT DOWNLOADED);
